# -----------------------------------------------------------------------------
# Title:         Differential Expression and PCA Plot in Amino Acid Level
# Author:        Hasan YILMAZ
# Date:          14/02/25
...
# -------------------------- Helper Functions ------------------------------ #
# 1. Collapse iso-acceptors and tRNAs to amino acid
...
single_factor_analysis <- function(arguments, analysis, seq_type){
  ...
  write.table(x = result, file = glue('{arguments$output}/{arguments$prefix}{toupper(seq_type)}_amino_diffAbundances.tsv'),
              sep = "\t", quote = FALSE)
  ...
  ggsave(glue('{arguments$prefix}{toupper(seq_type)}_amino_pca.pdf'), plot = pca,
  ...
  ggsave(glue('{arguments$prefix}{toupper(seq_type)}_amino_volcano.pdf'), plot = volcano,
...
two_factor_analysis <- function(arguments){
  ...
  write.table(x = result, file = glue('{arguments$output}/{arguments$prefix}Type*Condition_amino_diffAbundances.tsv'),
              sep = "\t", quote = FALSE)
  ...
  pca <- plotPCA(vsd, intgroup = "Type") +
    ...
    theme_pubr(border = T) +
    theme(legend.position = "none", aspect.ratio = 1)
  ggsave(glue('{arguments$prefix}Type*Condition_amino_pca.pdf'), plot = pca,
         device = 'pdf', path = arguments$output, width = 8, height = 8)
  ...
  ggsave(glue('{arguments$prefix}Type*Condition_amino_volcano.pdf'), plot = volcano,
         device = 'pdf', path = arguments$output, width = 8, height = 12)
